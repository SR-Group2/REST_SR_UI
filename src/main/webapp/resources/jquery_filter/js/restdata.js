    ...
    var addRestFile = [];
    // Old File Name
    var deletedImageRest = [];
    // Input File Element
    var restGallery;

    //Example 2
    function initRestaurantGallery(fileElement, uploadImagesArr){
    	restGallery = $(fileElement)[0];
        $(fileElement).filer({
            ...
            afterShow: function(){
                $.each(restGallery.files, function(index, file) {
                    addRestFile.push(file);
                });
            },
            onRemove: function(itemEl, currentFile, id, listEl, boxEl, newInputEl, inputEl){
                ...
                if (isImageFromServer){
                    ...
                    deletedImageRest.push(currentFile.name);
                } else {
                    var temp;
                    $.each(addRestFile, function(index, item) {
                        if (item.name === currentFile.name) {
                            temp = item;
                            return;
                        }
                    });
                    addRestFile.splice(addRestFile.indexOf(temp), 1);                
                }
            },
            onEmpty: null,
            captions: {
